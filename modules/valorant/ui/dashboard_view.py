import disnake
import re
from modules.valorant.services import valo_logic, match_manager, db_manager
from modules.valorant.services import custom_invite, guild_setup
...
class MatchDashboardView(disnake.ui.View):
    def __init__(
        ...
        *,
        ...
        self._teams_split = False

        for child in self.children:
            if child.custom_id == "dash_start":
                child.disabled = True
                child.style = disnake.ButtonStyle.gray
            if child.custom_id == "dash_team_reroll":
                child.disabled = True
    ...
    def build_embed(self) -> disnake.Embed:
        ...
        if self.team1 or self.team2:
            grade, score = valo_logic.team_balance_grade(self.team1, self.team2, db_data)
            balance_val = f"**{grade}** · {score}/100"
            roster_note = valo_logic.roster_imbalance_note(self.team1, self.team2)
            if roster_note:
                balance_val += f"\n-# {roster_note}"
            embed.add_field(name="⚖️ Баланс", value=balance_val, inline=False)
    ...
    def _split_teams(self):
        self.team1, self.team2 = valo_logic.split_teams_balanced(self.players)
        self._teams_split = True

    def _enable_team_reroll(self):
        for child in self.children:
            if child.custom_id == "dash_team_reroll":
                child.disabled = False
    ...
    async def team_btn(self, button: disnake.ui.Button, inter: disnake.MessageInteraction):
        ...
        self._enable_team_reroll()
        await self.update_message(inter)

    @disnake.ui.button(
        label="🔄 КОМАНДЫ",
        style=disnake.ButtonStyle.secondary,
        custom_id="dash_team_reroll",
        disabled=True,
        row=0,
    )
    async def team_reroll_btn(self, button: disnake.ui.Button, inter: disnake.MessageInteraction):
        if inter.author.id != self.host.id:
            return await inter.response.send_message("Только хост!", ephemeral=True)
        if not self._teams_split:
            return await inter.response.send_message("Сначала нажми **TEAM**.", ephemeral=True)
        self._split_teams()
        await self.update_message(inter)
