"""
match_manager.py — хранение активных матчей с персистентностью в JSON.

При перезапуске бота активные матчи восстанавливаются из файла.
Голосовые каналы при этом могут быть уже удалены — это нормально,
бот просто не будет их трогать при /finish.
"""
...
_MATCHES_PATH = os.path.join(os.path.dirname(__file__), "..", "data", "active_matches.json")
