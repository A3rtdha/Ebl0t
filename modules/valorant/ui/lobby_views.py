...
import asyncio
import logging
from modules.valorant.ui.dashboard_view import MatchDashboardView
from modules.valorant.services import guild_setup, db_manager, riot_api, custom_invite
...
class WaitingRoomView(disnake.ui.View):
    ...
    async def all_ready_btn(self, button: disnake.ui.Button, inter: disnake.MessageInteraction):
        ...
        if members_in_vc:
            players_list_obj = list(members_in_vc)
        elif self.dev_mode:
            players_list_obj = [inter.author]
        else:
            return await inter.response.send_message(
                "В голосовом **Ожидание** никого нет — зайди с игроками и нажми снова.",
                ephemeral=True,
            )

        if not self.dev_mode and len(players_list_obj) < 2:
            return await inter.response.send_message(
                "В «Ожидании» должно быть **минимум 2** игрока.",
                ephemeral=True,
            )
