"""Создание кастомок Valorant: /custom, /dev."""
...
from core.network_retry import retry_on_network

DEV_OWNER_ID = 634414560266158112  # /dev — только этот пользователь
...
class Customs(commands.Cog):
    ...
    async def _open_custom_setup(
        ...
        *,
        ...
        await retry_on_network(lambda: inter.response.defer())

        title = "🔧  DEV · кастомка" if dev_mode else "🎮  Кастомка Valorant"
        extra = (
            f"\n{ui_theme.DIVIDER}\n"
